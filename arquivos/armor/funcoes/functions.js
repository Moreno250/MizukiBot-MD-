...
const getGroupAdmins = (participants) => {
  ...
  for (const i of participants) {
    if (i.admin === 'admin' || i.admin === 'superadmin') {
      if (i.phoneNumber) admins.push(jidNormalizedUser(i.phoneNumber));
      if (i.id) admins.push(jidNormalizedUser(i.id));
      if (i.lid) admins.push(jidNormalizedUser(typeof i.lid === 'string' ? i.lid : i.lid._serialized));
    }
  }
  return admins;
};
...
const getMembros = (participants) => {
  ...
  for (let i of participants) {
    if (i.admin == null) {
      membros.push(i.phoneNumber || i.id);
    }
  }
  return membros;
};
...
// ===================== BANNERS =====================
// Banner principal (nome do bot) — gradiente esmeralda/ciano, fonte mais "cheia"
const banner2 = cfonts.render(("MIZUKI"), {
  font: 'block',
  align: 'center',
  gradient: ['#00ffcc', '#0077ff'],
...
function temporizador(segundos){
  ...
  var segundos2 = Math.floor(segundos % 60);
  return `${tempo(horas)}:${tempo(minutos)}:${tempo(segundos2)}`;
}
...
module.exports = { getBuffer, fetchJson, fetchText, generateMessageID, getGroupAdmins, getMembros, getRandom, banner2, banner3, temporizador, color, recognize, bgcolor, isFiltered, addFilter, chyt, getExtension, convertSticker, getpc, FormData }
